...
std::vector<float> calc_scale_factors_param(const unsigned int num_scale_levels, const float scale_factor) {
...
std::vector<float> calc_inv_scale_factors(const unsigned int num_scale_levels, const float scale_factor) {
...
std::vector<float> calc_inv_level_sigma_sq(const unsigned int num_scale_levels, const float scale_factor) {
    float scale_factor_at_level = 1.0;
...
void orb_extractor::calc_scale_factors() {
    scale_factors_ = calc_scale_factors_param(num_levels_, scale_factor_);
...
void orb_extractor::initialize() {
    ...
    double desired_num_keypts_per_scale = max_num_keypts_ * (1.0 - 1.0 / scale_factor_) /
    ...
    num_keypts_per_level_.at(num_levels_ - 1) = std::max(
            static_cast<int>(max_num_keypts_) - static_cast<int>(total_num_keypts), 0);
